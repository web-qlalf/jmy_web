...
<br />
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          ...
          <form class="form-horizontal" action="/admin_product/update/<?=$update_info[0]['no']?>" method="post">
            ...
            <div class="form-group ">
              ...
              <div class="card-body">
                  <div class="row" id="img_rows">
                    <!-- <div class="col-md-3">
                      <div class="form-group">
                        <div class="custom-file">
                          <input type="file" class="custom-file-input img_file" name="img_file[]" id="img_file_0" onChange="file_click(0)" />
                          <label class="custom-file-label img_file_url"><?=$img_info[0]['filename']?></label>
                          <input type="hidden" class="img_src_data" name="img_src_data[]" value="<?=$img_info[0]['fileURL'].$img_info[0]['filename']?>" />
                        </div>
                      </div>
                      <div class="p-2 border mb-4">
                        <div class="upload-images d-flex">
                          <div class="col-6">
                            <img class="img_src" src="<?=$img_info[0]['fileURL'].$img_info[0]['filename']?>" alt="img" class="w73 h73 border p-0">
                          </div>
                          <?php
                          $cnt = count($img_info);
                          for ($i=0; $i < $cnt; $i++) {
                          $image_volume[$i] = filesize($_SERVER['DOCUMENT_ROOT'].$img_info[0]['fileURL'].$img_info[0]['filename']) / 1024;
                          $imageSize[$i]  = getimagesize($_SERVER['DOCUMENT_ROOT'].$img_info[0]['fileURL'].$img_info[0]['filename']);
                          //  파일의 크기를 알아낸 후 1024로 나누어 kbyte 단위로 계산

                          $image_volume[$i]  = floor($image_volume[$i]);
                          // 소수점 제거를 위해 floor를 사용
                          }
                           ?>
                          <div class="col-6 ml-3 mt-2" >
                            <h6 class="mb-0 mt-1 font-weight-bold img_file_width">가로: <?=$imageSize[0][0]?>px</h6>
                            <h6 class="mb-0 mt-1 font-weight-bold img_file_height">세로: <?=$imageSize[0][1]?>px</h6>
                            <h6 class="mb-0 mt-1 img_file_volume">용량: <?=$image_volume[0]?>KB</h6>
                    ...
                    $cnt = count($img_info);
                    for ($i=0; $i < $cnt; $i++) {
                      $image_volume[$i] = filesize($_SERVER['DOCUMENT_ROOT'].$img_info[$i]['fileURL'].$img_info[$i]['filename']) / 1024;
                      $imageSize[$i]  = getimagesize($_SERVER['DOCUMENT_ROOT'].$img_info[$i]['fileURL'].$img_info[$i]['filename']);
                      //  파일의 크기를 알아낸 후 1024로 나누어 kbyte 단위로 계산
                    ...
                    ?>
                      <div class="col-md-3">
                        <div class="form-group">
                          <div class="custom-file">
                            <input type="file" class="custom-file-input img_file" name="img_file[]" id="img_file_<?=$i?>" onChange="file_click(<?=$i?>)" >
                            <label class="custom-file-label img_file_url"><?=$img_info[$i]['filename']?></label>
                            <input type="hidden" class="img_src_data" name="img_src_data[]" value="<?=$img_info[$i]['fileURL'].$img_info[$i]['filename']?>" />
                          </div>
                        </div>
                        <div class="p-2 border mb-4">
                          <div class="upload-images d-flex">
                            <div class="col-6">
                              <img class="img_src" src="<?=$img_info[$i]['fileURL'].$img_info[$i]['filename']?>" alt="img" class="w73 h73 border p-0">
                            </div>
                            <div class="col-6 ml-3 mt-2">
                              ...
                              <h6 class="mb-0 mt-1 img_file_volume">용량: <?=$image_volume[$i]?></h6>
                      ...
                      <script>
                        $('#img_file_<?=$i?>').attr('disabled', false);
                        var cnt = <?=$cnt?>;
                        if(cnt < 4){
                          for (var i = cnt; i < 4; i++) {
                            var img_file_input = $("#img_file_input");
                            var img_rows = $("#img_rows");
                            var clone = img_file_input.clone();
                            clone.find("#img_file_2").attr("onChange","file_click("+cnt+")");
                            clone.find("#img_file_2").attr("id","img_file_"+cnt);
                            clone.find(".img_src_data").val("");
                            clone.find("#img_file_input").val("style", "");
                            img_rows.append(clone);
                            console.log(cnt);
                          }
                        }
                      </script>
                  ...
                  ?>
                     ...
                     <div class="col-md-3" style="display:none" id="img_file_input">
                       <div class="form-group">
                         <div class="custom-file">
    ...
    <?php
    //여기부터 할 차례
    // 카테고리 정보 가져와서 뿌려주고 선택하게 만들기
    // 상품 수정시 이미지 덮어쓰기 가능하게 만들기
    // 상품 이미지 삭제시 확인창-> ajax 바로 삭제(복구 안 된다는 멘트 넣기)
      if(isset($category)){
        echo '<script>';
        echo "alert('카테고리 정보 있습니다.')";
        echo '</script>';
        var_dump($category);
      }else if(!isset($category)){
        echo '<script>';
        echo "alert('카테고리 정보 없습니다.')";
        echo '</script>';
      }else{} ?>
    </div>
    <script>
      ...
      function category_view(){
        ...
        $.ajax({
            url : "/admin_product/category_view",
            type : "post",
      ...
      function output_view(num){
        ...
        $.ajax({
            url : "/admin_product/delivery_view",
            type : "post",
            dataType:"json",
            success : function(response) {
              console.log(response);
      ...
      function delivery_info(select,num){
        ...
        $.ajax({
            url : "/admin_product/delivery_jnfo_view",
            type : "post",
