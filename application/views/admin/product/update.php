      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <form class="form-horizontal" action="/admin/product/updateInfo/<?=$update_info[0]['no']?>" method="post">
            ...
            <input type="hidden" id="product_no" name="product_no" value="<?=$update_info[0]['no']?>" readonly required/>
            <input type="hidden" id="inoutput_info_no" name="inoutput_info_no" value="<?=$update_info[0]['inoutput_info_no']?>" readonly required/>
                    ...
                    <div class="col-md-9">
                      <div class="card" style="box-shadow:0 0 0 0">
                          ...
                          <div class="row" id="optionform">
                            <div class="col-md-1">
                              <input type="text" class="form-control single_option_no" value="" />
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-3">
                              <input type="text" class="form-control single_option_stock_use1" name="productDetailList[0][stock_use1]" onchange="singleStock()" placeholder="500" required/>
                            </div>
                            <div class="col-md-2">
                              <button type="button" class="btn btn-pink btn-block waves-effect waves-light" id="optionSingleUpdate" onclick="optionSingleUpdate2()" disabled>수정하기</button>
                            </div>
                          </div>
      									</div>
                      </div>
                    ...
                    <div class="col-md-9">
                        ...
                        <div class="card-footer">
                          <div class="row mt-3">
                            ...
                            <div class="col-md-4">
                              <button type="button" class="btn btn-pink btn-block waves-effect waves-light" id="optionUpdate" onclick="optionDetailUpdate()" disabled>수정하기</button>
                            </div>
                            <div class="col-md-4">
                    ...
                    <div class="col-md-3">
                     <div class="form-group">
                       <div class="custom-file">
                         ...
                         <input type="hidden" class="img_src_data" name="img_src_data[]" value="<?=$file_URL?>" />
                         <input type="hidden" class="img_exe" name="img_exe[]" value="<?=$file_EXE?>" />
                         <input type="hidden" class="img_name" name="img_name[]" value="<?=$file_name?>" />
                         <input type="hidden" class="img_no" name="img_name[]" value="<?=$img_no?>" />
                       </div>
                     </div>
        ...
        <table class="table table-striped">
            ...
            <tbody>
              <tr id="optionlistform" name="optionInfo">
                  ...
                  <td>
                      <button type="button" class="btn btn-sm btn-danger optionCombiDelBtn" id="optionCombiDelBtn">X</button>
                  </td>
              </tr>
...
<?php } ?>
    <script>
          ...
          var optionform2 = $('#optionform2');
            ...
            $("#optionCombine").click(function(e){
                ...
                combine_list.forEach(function(element, index){
                    ...
                    optioncode.attr('onChange','optUpdateBtn(0)');
                    add_price.attr('onChange','optUpdateBtn(0)');
      ...
      //옵션 디테일 & 코드 수정
      function optUpdateBtn(index){
        $('#optionUpdate').attr('disabled', false);
      }
      function singleStock(){
        $('#optionSingleUpdate').attr('disabled', false);
      }

      //옵션 코드
      function optCodeDelete(index){
        // alert('삭제?');
        if(confirm('삭제하시겠습니까?(복구 불가능)')){
          var option_no = parseInt(index);
          ...
          $.ajax({
              ...
              type : "post",
              data : {"option_no":option_no,"product_no":product_no},
              dataType:"json",
              success : function(response) {
                if(response.ret == "succ") {
                  alert('삭제성공');

                  var opt_length = $('.option_no').length;
                  for (var i = 0; i < opt_length; i++) {
                    var opt_val = $('.option_no').eq(i).val();
                    if(opt_val == index){
                    $('.option_no').eq(i).parents('tr').remove();}
                  }
      ...
      //옵션 코드(추가가격 수정) && 상품디테일(재고)
      function optionDetailUpdate(){

        var optionCombiList = $('#optionCombiList');
        var optionTr = optionCombiList.children('tr');
        var option_num =optionTr.eq(0).children('td').eq(0).children('input').val();
        var option_no = [];
        var option_addPrice = [];
        var option_stock = [];
        var product_no = $('#product_no').val();
        ...
        if(option_num == '' || option_num == null ){
          ...
          for (var i = 0; i < option_name.length; i++) {
            opt_name[i] = option_name.eq(i).val();
            opt_dtname[i] = detail_name.eq(i).val();
          }

          // var option_no = $('.option_no');
          // var optioncode = $('.optioncode');
          // var add_price = $('.add_price');
          // var stock_num = $('.stock_num2');
          var add_price2 =[];
          var stock_num2 =[];
          var optioncode2 = [];

          for (var i = 0; i < optionTr.length; i++) {
            optioncode2[i] = optionTr.eq(i).children('td').eq(1).children('input').val();
            add_price2[i] = optionTr.eq(i).children('td').eq(2).children('input').val();
            stock_num2[i] = optionTr.eq(i).children('td').eq(3).children('input').val();
          }
          // console.log('add_price2: '+add_price2);
          // console.log('stock_num2: '+stock_num2);
          // console.log('optioncode2: '+optioncode2);
            $.post("/admin/option/totalUpdate", {
              'product_no' : product_no,
              'opt_name' : opt_name,
              'opt_dtname' : opt_dtname,
              'optioncode' : optioncode2,
              'add_price' : add_price2,
              'stock_num' : stock_num2
              }, function(data){
              // console.log('option_no: '+ option_no[i] +' option_code '+data);
              if(data == 0){
                console.log('option_code 수정 실패');
              }else{
                console.log('option_code 수정 성공');
              }
            });
          ...
          for (var i = 0; i < optionTr.length; i++) {
            ...
            option_no[i] = optionTr.eq(i).children('td').eq(0).children('input').val();
            option_addPrice[i] = optionTr.eq(i).children('td').eq(2).children('input').val();
            option_stock[i] = optionTr.eq(i).children('td').eq(3).children('input').val();

            $.post("/admin/option/update", {
              'option_no' : option_no[i],
              'option_addPrice' : option_addPrice[i],
              'option_stock' : option_stock[i]
              }, function(data){
              // console.log('option_no: '+ option_no[i] +' option_code '+data);
              if(data == 0){
                console.log('option_code 수정 실패');
              }else if(data == 3){
                console.log('option_code 수정 안 해도 됨');
              }else{
                console.log('option_code 수정 성공');
              }
            });
            $.post("/admin/productDetail/update", {
              'option_no' : option_no[i],
              'option_addPrice' : option_addPrice[i],
              'option_stock' : option_stock[i]
              },  function(data){
              // console.log('option_no: '+option_no[i]+' productDetail '+data);
              if(data == 0){
                console.log('option_code 수정 실패');
              }else if(data == 3){
                console.log('option_code 수정 안 해도 됨');
      ...
      function optionSingleUpdate2(){

        var single_opt_no= $('.single_option_no').val();


        if(single_opt_no == null || single_opt_no == '')
        {
          var single_stock= $('.single_option_stock_use1').val();
          var single_opt_no= $('.single_option_no').val();
          var single_name =  $('.single_option_List').val();
          var product_no = $('#product_no').val();
          var single_option_List = $('.single_option_List').val();
            $.post("/admin/option/single_add_update", {
              'option_addPrice' : 0,
              'single_name' : '단품',
              'single_stock' : single_stock,
              'procuct_no' : product_no
              }, function(data){
              if(data == 0){
                console.log('option_code 수정 실패');
              }else if(data == 3){
            ...
            // var single_option_List = $('.single_option_List').val();
            //
            // $.post("/admin/option/opt_update", {
            //   'product_no' : product_no,
            //   'opt_name' : single_option_List,
            //   'opt_dtname' : single_option_List
            //
            //     }, function(data){
            //   // console.log('option_no: '+ option_no[i] +' option_code '+data);
            //   if(data == 0){
        ...
        else
        {
          ...
          var product_no = $('#product_no').val();
          var single_option_List = $('.single_option_List').val();
          console.log('아아악 넘어왔다고!!!!!!!!');
          // console.log('single_opt_no: '  + single_opt_no);
          // console.log('product_no: '  + product_no);
          // console.log('single_stock: '  + single_stock);

          $.post("/admin/option/single_update", {
            'product_no' : product_no,
            'option_no' : single_opt_no,
            'option_stock' : single_stock
          }, function(data){
              console.log('넘어옴?');
              console.log(data);
              if(data == 0){
                console.log('product_detail 수정 실패');
              }else if(data == 3){
                console.log('product_detail 수정 안 해도 됨');
              }else{
                console.log('product_detail 수정 성공');
              }
          });
      }
    }
      function optionSingleCode(){
        var single_opt_no= $('.single_option_no').val();
        var single_name =  $('.single_option_List').val();
        var single_stock= $('.single_option_stock_use1').val();
        var product_no = $('#product_no').val();
        ...
        $.post("/admin/option/add", {
          ...
          if(data == null || data == ''){
            console.log('option_code 등록 실패');
            return false;
          }else{
            console.log('option_code 등록 성공');
            var single_no = data.replace(/\"/gi, "");
             $('.single_option_no').val(single_no);
             return true;
          }
        });
        console.log('option_code 등록 끝');
      }
      //조합 알고리즘
      ...
      function option_update_select(){
        ...
        if(option_name == '단품'){
          var code_num = '<?php echo $product_detail[0]['option_code_no']; ?>';
          ...
          $('.single_option_stock_use1').attr('disabled', false);
          $('.single_option_stock_use1').val(single_stock);
          $('.single_option_no').val(code_num);
          ...
          $('.single_option_stock_use1').attr('disabled', true);
          ...
          var optionlistform = $('#optionlistform');
            ...
            for (var k = 0; k < option_stock4; k++) {
              var clone = optionlistform.clone();
              var optcode = $('.optioncode').eq(k).val();
              // console.log(k+' optcode는 ' + $('.optioncode').eq(k).val());
              // console.log(k+'는 '+option_code3[k]['name']);
                    var addprice = option_code3[k]['price'];
                    var optioncode = option_code3[k]['name'];
                    var stock_num2 = option_stock3[k]['stock'];
                    var option_code_no = option_stock3[k]['option_code_no'];
                    // $('.add_price').eq(k).val(addprice);
                    // $('.stock_num2').eq(k).val(stock_num2);
                    // $('.option_no').eq(k).val(option_code_no);
                    clone.find(".optioncode").val(optioncode);
                    clone.find(".add_price").val(addprice);
                    clone.find(".stock_num2").val(stock_num2);
                    clone.find(".option_no").val(option_code_no);

                    clone.find('#optionCombiDelBtn').attr('onclick', 'optCodeDelete('+option_code_no+')');

                    clone.find('.add_price').change(function(e){
                        e.preventDefault();
                        optUpdateBtn(option_code_no);
                    });
                    clone.find('.stock_num2').change(function(e){
                        e.preventDefault();
                        optUpdateBtn(option_code_no);
                    });
                    $('#optionCombiList:last').append(clone);
                    // console.log(addprice);
                    // console.log(stock_num2);
                    // console.log(option_code_no);
                  ...
                  console.log('대분류');
                      ...
                      var result = response[0].forEach(function (value, index, array) {
                        if(value.id == main_id){
                          ...
                          var opt_val = value.id;
                          $('.main_category_value').val(opt_val);
                        }else{
                  ...
                  console.log('대/중분류');
                      ...
                      var result2 = response[1].forEach(function (value, index, array) {
                        if(value.id == middle_id){
                          var option = "<option value='"+value.id+"' selected>"+value.name+"</option>";
                          var opt_val = value.id;
                          $('.middle_category_value').val(opt_val);
                        }else{
                          var option = "<option value='"+value.id+"'>"+value.name+"</option>";
                        }
                        console.log("?");
                  ...
                  case 3://대분류,중분류,소분류
                      ...
                      var result3 = response[2].forEach(function (value, index, array) {
                        if(value.id == sub_id){
                          var option = "<option value='"+value.id+"' selected>"+value.name+"</option>";
                          var opt_val = value.id;
                          $('.sub_category_value').val(opt_val);
                        }else{
                          var option = "<option value='"+value.id+"'>"+value.name+"</option>";
                        }
                        $('.sub_category_select:last').append(option);
      ...
      function option_select(){
        ...
        switch (option_select) {
          case '단품':
            if(confirm('기존 옵션은 지워집니다. 그래도 단품으로 바꾸시겠습니까?')){
              $('.option_setting_single').attr('style','');
              $('.option_setting_multi').attr('style','display:none');
              $('.option_value_1').val('0');
              $('.option_value_2').val('');
              $('.option_value_1').attr('readonly',true);
              $('.single_option_stock_use1').attr('disabled', false);
              $('.single_option_no').attr('disabled', false);
              $('.single_option_List').attr('disabled', false);
              $('.option_name').attr('disabled', true);
              $('.detail_name').attr('disabled', true);
              $('.single_option_stock_use1').val('');
              $('.single_option_no').val('');
              $('.single_option_stock_use1').val('');
              ...
              var option_code_no = [];
              for (var i = 0; i < optionTr.length; i++) {
                option_code_no[i] = optionTr.eq(i).children('td').eq(0).children('input').val();
              }

              $.post("/admin/option/delete_update", {
                'product_no' : product_no,
                'option_code_no' : option_code_no
              },  function(data){
                if(data == 0){
                  console.log('option 삭제 실패');
                }else if(data == 3){
                  console.log('option 삭제  할 게 없음');
                }else{
                  console.log('option 삭제 성공');
                  console.log(data);
                }
              });
              ...
              $('.option_select option').eq(1).attr('selected', false);
              $('.option_select option').eq(2).attr('selected',true);

            }
            break;
          case '옵션':
          if(confirm('기존 옵션은 지워집니다. 그래도 옵션으로 바꾸시겠습니까?')){
            $('.option_setting_single').attr('style','display:none');
            $('.option_setting_multi').attr('style','');
            $('.option_value_1').val('');
            $('.option_value_2').val('');
            ...
            $('.option_name').attr('disabled', false);
            $('.detail_name').attr('disabled', false);
            $('.single_option_no').attr('disabled', true);

            var single_name = '<?php echo $option_info[0]['name']; ?>';
            var single_stock = '<?php echo $product_detail[0]['stock']; ?>';
            var code_num = '<?php echo $product_detail[0]['option_code_no']; ?>';
            var detail_num = '<?php echo $product_detail[0]['no']; ?>';
            var product_no = $('#product_no').val();

            $.post("/admin/option/delete_update", {
              'product_no' : product_no,
              'option_code_no' : code_num
            },  function(data){
              if(data == 0){
                console.log('option 삭제 실패');
              }else if(data == 3){
            ...
            $('.option_select option').eq(1).attr('selected',true);
            $('.option_select option').eq(2).attr('selected',false);
      ...
      function file_update_click(num){
        ...
        if(confirm('기존 이미지는 삭제됩니다. 동의하십니까?')){
          ...
          formData.append("img_code_no", $(".img_no").eq(num).val());
          formData.append("img_exe_bf", $(".img_exe").eq(num).val());
